$(function() {
    ...
    function loadNextStepStatus(event) {
        ...
        let arrListStatus = [
            ...
            { status: 'Đặt hàng thành công chờ xử lý', nextstep: 'Bạn có muốn chuyển đơn hàng sang trạng thái đã tiếp nhận đơn hàng?' },
            { status: 'Đã tiếp nhận', nextstep: 'Bạn có muốn chuyển đơn hàng sang trạng thái chờ xuất kho?' },
            { status: 'Chờ xuất kho', nextstep: 'Bạn có muốn chuyển đơn hàng sang trạng thái đang vận chuyển?' },
            { status: 'Đang vận chuyển', nextstep: 'Bạn có muốn chuyển đơn hàng sang trạng thái hoàn thành?' },
            { status: 'Hoàn thành', nextstep: 'Bạn có muốn chuyển đơn hàng sang trạng thái đã thu tiền?' },
            { status: 'Đã thu tiền', nextstep: 'Đơn hàng đã thu tiền' },
        ]
        ...
        if (statusCurrent > 0 && statusCurrent < 6) {
        ...
        Swal.fire(swalOption).then((result) => {
            if (result.isConfirmed && statusCurrent > 0 && statusCurrent < 6) {
                $.ajax({
                    type: "GET",
                    url: urlRequest,
                    success: function(data) {
    ...
    // js load ajax chuyển trạng thái đơn hàng
    $(document).on("click", ".condition span", loadNextStepCondition);

    function loadNextStepCondition(event) {
        event.preventDefault();
        let statusWrap = $(this).parents('.condition');
        // get url load ajax
        let urlRequest = statusWrap.data("url");
        // get giá trị status hiện tại
        let statusCurrent = parseInt($(this).data("condition"));
        ...
        Swal.fire(swalOption).then((result) => {
            if (result.isConfirmed && statusCurrent >= 0 && statusCurrent < 3) {
                $.ajax({
                    type: "GET",
                    url: urlRequest,
                    success: function(data) {
