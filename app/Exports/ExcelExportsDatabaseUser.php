...
class ExcelExportsDatabaseUser implements FromArray, WithHeadings, WithStyles, WithEvents, ShouldAutoSize
{
...
// dd($this->start,$this->end);
        //if($this->typeUser==-1){
            $user=$this->model->where('type', 1)->select($this->selectField)->get();
        // }else{
        //     $user=$this->model->where('type', 1)->where('active',$this->typeUser)->select($this->selectField)->get();
        // }
        $point = new Point();
// dd($user);
        if($user->count()>0){
