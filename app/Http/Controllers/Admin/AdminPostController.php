...
class AdminPostController extends Controller
{
    ...
    public function store(ValidateAddPost $request)
    {
        ...
        try {
            ...
            foreach ($this->langConfig as $key => $value) {
                ...
                $itemPostTranslation['time'] = $request->input('time_' . $key);
                $itemPostTranslation['dia_diem'] = $request->input('dia_diem_' . $key);
                $itemPostTranslation['range'] = $request->input('range_' . $key);
                $itemPostTranslation['value'] = $request->input('value_' . $key);
    ...
    public function update(ValidateEditPost $request, $id)
    {
        try {
            ...
            foreach ($this->langConfig as $key => $value) {
                ...
                $itemPostTranslationUpdate['time'] = $request->input('time_' . $key);
                $itemPostTranslationUpdate['dia_diem'] = $request->input('dia_diem_' . $key);
                $itemPostTranslationUpdate['range'] = $request->input('range_' . $key);
                $itemPostTranslationUpdate['value'] = $request->input('value_' . $key);
    ...
    public function destroy($id)
    {
        ...
        Key::where('key_id', $post->id)->delete();
