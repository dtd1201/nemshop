@if($data)
<form  action="{{ route('admin.agency.storeChangeTienNo',['id'=> $data->id]) }}"  data-url="{{ route('admin.agency.storeChangeTienNo',['id'=> $data->id]) }}" data-ajax="submit" data-id='{{$data->id}}' data-target="alert" data-href="#modalAjax" data-content="#content" data-method="POST" method="POST">
    @csrf
